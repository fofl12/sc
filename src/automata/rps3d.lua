...
local function simulate(left, right, up, down, front, back)
  if left == right and right == up and up == down and down == front and front == back then
    return left
  end
  do
    local n = 0
    local ok = true
    for _, x in next, {left, right, up, down, front, back} do
      if x ~= 0 then
        if n ~= x and n ~= 0 then
          ok = false
          break
...
while true do
  ...
  for y, xlist in ipairs(map) do
    for x, zlist in ipairs(xlist) do
      task.wait(1/20)
      for z, val in ipairs(zlist) do
        if val ~= 0 then
          local pixel = Instance.new('WedgePart')
          pixel.Size = Vector3.one
          pixel.Position = Vector3.new(x - height / 2, y, z + 100)
          pixel.Anchored = true
